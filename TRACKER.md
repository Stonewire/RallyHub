...
- [x] **SEC-1 Phase 1 security hardening** (on `security-fixes` as V2.4.8): tablet event lists are token-gated server-side, Auth profile creation no longer trusts user-editable role/org metadata, organization-logo storage writes are org-scoped, broad public bucket listing policies are removed, and the first batch of obvious admin/trigger `SECURITY DEFINER` functions no longer inherit implicit `PUBLIC` execute. See `docs/SECURITY-REVIEW-2026-07.md`.
...
- [ ] **SEC-3 Index/query advisor cleanup**: add missing FK indexes and the targeted `submissions(event_id, created_at desc)` query index; defer dropping "unused" indexes until after a real usage window.
