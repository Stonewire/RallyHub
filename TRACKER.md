...
- [ ] **CONTACT-1** Marketing demo form has NO server-side destination yet. It currently composes a pre-filled email via the visitor's own mail client (safe, no third-party transmission). Decide the real backend: edge function → Resend (ties into EMAIL-1), a booking tool, CRM, or the support-ticket workflow. Constant `CONTACT_EMAIL` (`hello@rallyhubapp.com`) in `DemoContactSection.tsx` is the current fallback — confirm that address is right (note: domain is now rallyhub.games).
