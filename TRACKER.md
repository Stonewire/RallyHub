...
- [ ] **SEC-5 New advisor findings (2026-07-11)**: `organizations_insert_authenticated` INSERT policy is always-true (any logged-in user can create orgs — decide if register-client should be the only path); enable leaked password protection in Auth settings (one dashboard toggle); 14 functions with mutable search_path; delete dead deployed edge functions (`smooth-api` hello-world template, `invite-member`, `reveal-bingo-winner`) and unused local `create-facilitator`.
