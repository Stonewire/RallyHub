/** Shared with src/lib/bingo-engine.ts — keep in sync for edge functions. */

export type BingoTrack = { id: string; title: string; artist: string }
export type BingoTeam = { id: string; name: string | null }
export type BingoCell = { trackId: string; title: string; artist: string }
...
export function generateBingoRun(opts: {
  ...
  const winTrackIds = shuffledTracks.slice(0, 5).map((t) => t.id)
  ...
  const playOrder = buildPlayOrder(winTrackIds, tracks.map((t) => t.id), targetPlayCount, seed + 99)

  return { winnerTeamId: winner.id, cardsByTeamId, playOrder }
}
