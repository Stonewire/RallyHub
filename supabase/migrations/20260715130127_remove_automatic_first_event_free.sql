-- Event discounts must be explicit. Paid plans no longer receive an automatic
-- first-event comp; selected clients can still be given a 100% event promo code.
--
-- Replaces the invoice function from 071_first_event_free_paid_plans while
-- retaining Partner/Enterprise comping, event promo codes, and the approved
...
begin
  ...
  if v_plan = 'partner' then
    ...
    -- Apply the best active event promo code, if any. A selected client can be
    -- given a 100% code when RallyHub wants to offer a complimentary event.
    ...
    -- Approved educational organisations receive another 50% off after any
    -- event promo code has been applied.
...
-- The activation trigger calls this as the function owner. Client sessions do
-- not need direct access to this SECURITY DEFINER function; leaving it callable
-- would let an authenticated account create another org's invoice or consume its
-- promo code by guessing an event UUID.
