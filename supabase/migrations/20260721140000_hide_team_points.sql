-- Lets a facilitator hide each team's point total from their own device,
-- independent of "show_scores" (which only controls the audience display).
alter table event_state
  add column if not exists hide_team_points boolean not null default false;
