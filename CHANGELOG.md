...
- **Live-event fix.** A recurring event's second run could be auto-archived
  within 15 minutes of going live. The 12-hour auto-archive job measured from
  the event's first invoice, which a restart deliberately keeps, so from run 2
  onward the event looked 12 hours stale the moment it started. Archiving a live
  event stops any phone joining late or reloading from getting back in. The job
  now measures from when the current run went live.
- The tablet kiosk event list fix from V3.28.0 was written but never applied to
  the database, so tablets still listed Ready and binned events. Applied.
...
- The keyboard now clicks with Rumen's own recorded mechanical keyboard
  (CF7): four rotating key clicks plus dedicated space, submit and
  backspace sounds, cut from the 8 Aug recording, pitched down for depth,
  filtered and normalised. Decoded into Web Audio buffers for zero-latency
  playback; a synthesised thock stands in until they load.
...
- Photo and video challenges no longer offer an upload path. The Upload
  video button and its file picker are gone from the recorder; photos were
  already camera-only (the phone flows open the camera app directly). Teams
  capture in the moment or not at all.
...
- The camera-open diagnostics added in V3.1.4 never arrived: the diagnostics
  table only accepts rows carrying a live event's join token, and the camera
  library sent none, so every line was silently dropped. The event id now
  travels with each camera open. No behaviour change for players.
