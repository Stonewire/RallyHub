...
**Superseded in part by "Installable app: built, 2 Aug 2026" below.** The
manifest, icons, service worker and install buttons now exist. The launcher
idea and the three auth models are still open.
...
**Superseded by "Installable app: built, 2 Aug 2026" below. The "no web
manifest" gap described here is closed.**
...
## Installable app: built, 2 Aug 2026

Rumen asked for the site to install as an app on Chromium and Safari, across
Windows, Mac, iPhone, iPad and Android. Decisions he made at the start of the
job: always RallyHub branding, never per-client branding, so one manifest
serves every tenant; charcoal icon with the yellow mark; install buttons on
admin, facilitator, tablet and the participant join screen, not on marketing.
Mid-job he added that the tablet link matters most, since that is where most
devices will be installed.

**Two manifests, because start_url cannot be right for everyone.**
`manifest.webmanifest` starts at `/`, which suits admin and facilitator: the
root already routes by role, so the icon lands an admin on the admin panel and
a facilitator on their event list, and it keeps working after any one event
ends. `manifest-here.webmanifest` omits `start_url` and `id` entirely, so an
install pins the page it was made from and gets its own app identity.
`PageScopedManifest` swaps the link tag in on `/tablet/*` and `/join/*`, which
is what makes a kiosk icon reopen that kiosk and a player's icon reopen that
event. Two tablets can therefore hold two separate icons. This is the
"omitting start_url may solve the multi-entry-point problem" note from the
1 Aug brief, now tested rather than assumed, though only in a desktop Chromium.

**The service worker is deliberately almost empty.** Chromium will not offer to
install a site whose worker cannot answer a navigation offline, so one has to
exist. It caches `offline.html` and nothing else. Every request that is not a
top-level navigation is passed straight back to the browser untouched, so
Supabase, realtime, audio range requests and the ffmpeg worker never go through
it. A worker holding stale JavaScript during a live event is a far worse
failure than a page that will not load, which is why there is no precaching and
no app shell. `skipWaiting` plus `clients.claim` mean a deploy takes over
immediately instead of leaving an old worker in charge.

**Icons** are generated by `scripts/generate-pwa-icons.py` from the vector
logo, not from the 285x271 PNG, which would have had to be upscaled for the 512
sizes. The mark is cut from the lockup and repainted yellow from its alpha, then
centred on an opaque charcoal square. Opaque matters: iOS composites a
transparent icon onto black and Android does the same behind a maskable one.
Maskable icons sit the mark at 56% of the canvas, because Android only
guarantees the middle 80% circle survives its crop and a square's diagonal
eats the rest.

**Verified** on a production build in Chromium: manifest served as
`application/manifest+json`, all four icons resolving at their declared sizes,
worker registered and controlling the page, the offline page actually served
with the server stopped, the manifest swapping to the page-scoped one on join
and tablet and reverting on the way out, the button appearing only when
install is possible, surviving a dismissed prompt, and disappearing on
`appinstalled`. Not verified: a real iPhone, iPad or Android device, and
whether Chromium's own install menu lights up, which the Browser pane cannot
show.

**Open, for Rumen.** The tablet session lives in `sessionStorage`, so an
installed kiosk asks for the 4-digit password again every time the app is
closed and reopened. That is the right default for a shared device but it is
